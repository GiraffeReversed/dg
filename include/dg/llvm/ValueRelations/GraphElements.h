...
private:
    struct Supplements {
        ...
        std::vector<VREdge*> getNextEdges(VRLocation* loc) const {
            return direction == Dir::FORWARD ? loc->getSuccessors() : loc->getPredecessors();
        }
    ...
    protected:
        ...
        void increment() {
            ...
            for (VREdge* edge : getNextEdges(current)) {
                ...
                // do not explore if there is no target or if target was already explored
                // or if is in other function
                if (irrelevant(edge))
                    continue;
                ...
                if (direction == Dir::FORWARD && categorizedEdges && next->isJustBranchJoin()) {
        ...
        const std::queue<std::pair<VRLocation*, VREdge*>>& structure() const { return queue; }
    ...
    protected:
        ...
        DFSIncrement(const llvm::Function* f, VRLocation* start, bool /*e*/, Dir d)
        ...
        void increment() {
            while (!stack.empty()) {
                VRLocation* current; unsigned index; VREdge* prevEdge;
        ...
        const std::vector<std::tuple<VRLocation*, unsigned, VREdge*>>& structure() const { return stack; }
    ...
    struct VRFunctionIterator : public Strategy {
        ...
        VRFunctionIterator(const llvm::Function* f, VRLocation* start, bool e, Supplements::Dir d)
...
public:
    ...
    BFSIterator bfs_begin(const llvm::Function* f) const {
        return BFSIterator(f, &getEntryLocation(f), categorizedEdges, Dir::FORWARD);
    }
    ...
    BFSIterator bfs_end(const llvm::Function* /*f*/, VRLocation& /*start*/) const {
        return BFSIterator();
    }

    BFSIterator backward_bfs_begin(const llvm::Function* f, VRLocation& start) const {
        return BFSIterator(f, &start, categorizedEdges, Dir::BACKWARD);
    }

    BFSIterator backward_bfs_end(const llvm::Function* /*f*/, VRLocation& /*start*/) const {
        return BFSIterator();
    }

    DFSIterator dfs_begin(const llvm::Function* f) const {
        return DFSIterator(f, &getEntryLocation(f), categorizedEdges, Dir::FORWARD);
    }
...
private:
    struct VRCodeGraphIterator {
        ...
        friend bool operator==(const VRCodeGraphIterator& lt, const VRCodeGraphIterator& rt) {
            ...
            return (ltIsEnd && rtIsEnd) || (!ltIsEnd && !rtIsEnd && lt.intoFunction == rt.intoFunction);
        }
        ...
        VRCodeGraphIterator& operator++() {
            ...
            if (intoFunction == BFSIterator()) {
                ...
                if (intoMapping != endMapping)
                    intoFunction = BFSIterator(intoMapping->first,
                                                intoMapping->second,
                                                categorizedEdges,
                                                Dir::FORWARD);
            }
...
public:
    ...
    iterator begin() const {
        return functionMapping.empty() ? iterator()
            : iterator(functionMapping.begin(), functionMapping.end(), categorizedEdges);
    }

    iterator end() const {
        return functionMapping.empty() ? iterator() : iterator(functionMapping.end());
    }
};
