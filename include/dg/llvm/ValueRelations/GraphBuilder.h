...
struct GraphBuilder {
    ...
    void build(const llvm::BasicBlock& block) {
        ...
        for (; it != block.end(); ++it) {
            ...
            vrblock->last()->connect(newLoc, std::unique_ptr<VROp>(new VRInstruction(previous)));
...
struct GB {
    ...
    GB(const llvm::Module& m, VRCodeGraph& c)
    ...
    void buildSwitch(const llvm::SwitchInst* swtch, VRLocation& last) {
        ...
        for (auto& it : swtch->cases()) {
            ...
            last.connect(padding, new VRAssumeEqual(swtch->getCondition(), it.getCaseValue()));
